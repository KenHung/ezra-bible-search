...
def read_cbol_dict(path: str) -> pd.DataFrame:
    types = {"strong": "string", "defs": "string"}
    data = pd.read_csv(
        path, sep="^", names=list(types.keys()), usecols=[0, 1], dtype=types
    )
    return data
...
def create_name_dict(cbol_dict: pd.DataFrame) -> pd.DataFrame:
    ...
    left_over_vocabs = extract_vocabs(
        left_over.def_lines, f'({"|".join(left_over_names)})'
    ).drop_duplicates()

    all_vocabs = pd.concat([vocabs_eq, vocabs_over_def, left_over_vocabs])
    vocabs = all_vocabs.merge(
        cbol_dict[["strong", "pos", "jieba_pos"]], left_index=True, right_index=True
    )
...
def extract_vocabs(
    cbol_def_lines: pd.Series, pattern: str = r"(.*?)=(.*)"
) -> pd.DataFrame:
    vocab_defs = cbol_def_lines.explode().str.extract(pattern).dropna()
    return vocab_defs
...
def find_name_over_def(lines):
    first_def = next((n for n, line in enumerate(lines) if line.startswith("1)")), None)
